...
from models.DCQDet.DCQDet import build_DCQ_Det
...
def get_args_parser():
    parser = argparse.ArgumentParser('Set DCQ-Det detector', add_help=False)
    ...
    # TODO:center points generation parameters
    ...
    parser.add_argument('--center_thresh', type=float, default=0.1, help='threshold for centermap.')
...
def build_model_main(args):
    model, criterion, postprocessors = build_DCQ_Det(args)
...
def main(args):
    ...
    # TODO: TensorBoard
    writer = SummaryWriter('runs/DCQ-Det_coco_intern_1')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            args=args, logger=(logger if args.save_log else None), ema_m=ema_m, writer=writer)
        if args.output_dir:
            checkpoint_paths = [output_dir / 'checkpoint.pth']

        if not args.onecyclelr: # args.onecyclelr=false
            lr_scheduler.step()
        if args.output_dir:
            checkpoint_paths = [output_dir / 'checkpoint.pth']
