...
from .intern_image import build_intern_image
...
def build_backbone(args):
    ...
    if name in ['resnet50', 'resnet101']:
        ...
        assert return_interm_indices in [[0, 1, 2, 3], [1, 2, 3], [3]]
        backbone = Backbone(name, train_backbone, dilation,
                            return_interm_indices,
                            batch_norm=FrozenBatchNorm2d)
        bb_num_channels = backbone.num_channels
    elif name in ['swin_B_224_22k', 'swin_B_384_22k', 'swin_L_224_22k', 'swin_L_384_22k',
                'intern_T_22k', 'intern_L_22k', 'intern_XL_22k']:
        if name in ['swin_B_224_22k', 'swin_B_384_22k', 'swin_L_224_22k', 'swin_L_384_22k']:
            pretrain_imgsize = int(name.split('_')[-2])
            backbone = build_swin_transformer(name, pretrain_imgsize)
            num_channels = 2048
        elif name in ['intern_T_22k', 'intern_L_22k', 'intern_XL_22k']:
            backbone = build_intern_image(name, args)
            num_channels = 2048
