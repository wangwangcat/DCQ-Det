...
class DeformableTransformer(nn.Module):
    ...
    def gen_encdoer_output_proposals(self, memory, memory_padding_mask, spatial_shapes):
        N_, S_, C = memory.shape
        base_scale = 4.0
        proposals = []
        _cur = 0
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None):
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            ...
            pos_embed = pos_embeds.flatten(2).transpose(1, 2) # bs, hw, c
            # self.level_embed = nn.Parameter(torch.Tensor(num_feature_levels, d_model))
            lvl_pos_embed = pos_embed + self.level_embed[lvl].view(1, 1, -1)
            lvl_pos_embed_faltten.append(lvl_pos_embed)
            src_flatten.append(src)
            mask_flatten.append(mask)
        src_flatten = torch.cat(src_flatten, 1) # bs, h*w*scales, c
        mask_flatten = torch.cat(mask_flatten, 1) # bs, h*w*scales
        lvl_pos_embed_faltten = torch.cat(lvl_pos_embed_faltten, 1)
        spatial_shapes = torch.as_tensor(spatial_shapes, dtype=torch.long, device=src_flatten.device)
        level_start_index = torch.cat(spatial_shapes.new_zeros((1, )), spatial_shapes.prod(1).cumsum(0)[:-1])
        valid_ratios = torch.stack([self.get_valid_ratio(m) for m in masks], 1)

        # encoder
        memory = self.encoder(src_flatten, spatial_shapes, level_start_index, valid_ratios, lvl_pos_embed_faltten, mask_flatten)
